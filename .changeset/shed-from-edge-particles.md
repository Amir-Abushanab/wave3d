...
Adds `particles.shed` (`{ rate, drift, fromWave }`) as a second emitter mode alongside the ambient field, plus studio controls. The "Solar Plume" preset sheds off its plume. Behind `#ifdef SHED`, so field-only fields compile none of it and stay byte-identical when shed is off.
